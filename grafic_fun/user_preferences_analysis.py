...
def _crear_primera_grafica(parent, df, available_width, colors):
    ...
    _crear_grafico_top_generos_ventas(ax1, df, colors)
    _crear_grafico_preferencias_region(ax2, df, colors)
    _crear_grafico_top_consolas_ventas(ax3, df, colors)

    _aplicar_estilo_graficos([ax1, ax2, ax3])
...
def _crear_segunda_grafica(parent, df, available_width, colors):
    ...
    frame2 = _crear_frame_grafica(parent, "📈 Métricas específicas por críticas ", "#00ff88")
    ...
    _crear_grafico_mejores_consolas(ax4, df, colors)
    _crear_grafico_mejores_publishers(ax5, df, colors)
    _crear_grafico_top_juegos_genero(ax6, df)

    _aplicar_estilo_graficos([ax4, ax5])
